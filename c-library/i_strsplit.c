...
static	int		i_count_words(const char *str, char c)
{
	int	word;
	int	i;
...
static	char	*i_word(const char *str, char c, int *i)
{
	char	*s;
	int		k;

	if (!(s = (char *)malloc(sizeof(s) * (i_strlen(str)))))
		return (NULL);
	k = 0;
	while (str[*i] != c && str[*i])
	{
...
char			**i_strsplit(const char *str, char c)
{
	int		i;
	int		j;
	int		wrd;
